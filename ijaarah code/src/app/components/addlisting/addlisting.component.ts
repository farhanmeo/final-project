...
export class AddlistingComponent implements OnInit {
  ...
  uploadSingle() {
    this.spinner = 1;
    if (this.imageChecker != "true") {
      alert("Ad Request Failed");
      this.spinner = 0;
  ...
  pushUpload(upload: Employee) {
    ...
    upload.uid = this.uid;
    upload.price = this.postAdd.get('price').value;
    upload.isFeatured = "false";
    upload.isReserved = "false";
      ...
      () => {
        ...
        uploadTask.snapshot.ref.getDownloadURL().then(function (downloadURL) {
          ...
          if (itemsRef.push(upload)) {
            ...
            // aaa.resetForm(aaa.postAdd);

            aaa.router.navigate(['/dashboard/dashboard-content']);
